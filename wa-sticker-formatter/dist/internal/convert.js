...
const convert = (data, mime, { quality = 100, background = Utils_1.defaultBg, type = StickerTypes_1.StickerTypes.DEFAULT }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (isAnimated && ['crop', 'circle'].includes(type)) {
        ...
        [image, type] = [yield (0, crop_1.default)(filename), type === 'circle' ? StickerTypes_1.StickerTypes.CIRCLE : StickerTypes_1.StickerTypes.DEFAULT];
    }
    const img = (0, sharp_1.default)(image, { animated: type !== 'circle' }).toFormat('webp');
    if (type === 'crop')
        img.resize(512, 512, {
    ...
    if (type === 'circle') {
        img.resize(512, 512, {
            ...
            {
                input: Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 512 512"><circle cx="256" cy="256" r="256" fill="${background}"/></svg>`),
                blend: 'dest-in'
            }
        ]);
    }
